...
class Game:
    ...
    def paint(self):
        ...
        """
        root = tk.Tk()
        canvas = tk.Canvas(root, width = self._field.width()*10+10, height = self._field.height()*10+10)
        ...
        for coord in self._apples_positions:
            ...
            canvas.create_rectangle(coord[1]*10-5, coord[0]*10-5, coord[1]*10+5, coord[0]*10+5, fill = 'red')
        for coord in self._snake:
            screen[coord[0]][coord[1]] = SNAKE
            canvas.create_rectangle(coord[1]*10-5, coord[0]*10-5, coord[1]*10+5, coord[0]*10+5, fill = 'green')
        for line in screen:
            print(''.join(line))
        canvas.pack()
        root.mainloop()
